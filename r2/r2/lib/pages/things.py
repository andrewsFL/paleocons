...
class LinkButtons(PrintableButtons):
    def __init__(self, thing, comments = True, delete = True, report = True):
        ...
        show_share = ((c.user_is_loggedin or not g.read_only_mode) and thing.hidden and
                      not thing.subreddit.quarantine and
                      not thing.disable_comments and
                      not thing._deleted)

        show_sticky_comment = (thing.is_stickyable and
                                    (is_author and
                                        c.user.employee))
        ...
        show_givegold = thing.can_gild and (c.permalink_page or c.profilepage) and thing.hidden
...
class CommentButtons(PrintableButtons):
    def __init__(self, thing, delete = True, report = True):
        ...
        show_givegold =  (thing.can_gild and
                            (c.user.employee and  # Admin distinguish
                             (c.user.employee and
                                is_author)))
